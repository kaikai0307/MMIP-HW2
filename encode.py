...
from bitstream import save_compressed_file
...
def auto_level_shift(image, bit_depth=None, pixel_repr=None):
    """
    自動判斷是否需要 level shift：
    - Signed (PixelRepresentation=1) -> 不做 shift
    - Unsigned -> shift = 2^(B-1)
    若沒有 pixel_repr，則用 dtype/最小值推斷。
    """
    if bit_depth is None:
        bit_depth = np.iinfo(image.dtype).bits
    if pixel_repr is None:
...
def _first_or_value(value, default):
    if value is None:
        return default
...
def test_codec_mvp(
    ...
    if level_shift is None:
        level_shift = auto_level_shift(original_image, bit_depth, pixel_repr)
...
def encode_cmd(args):
    raw_img, header = analyze_dicom_file(args.input)
    ...
    entropy_method = EntropyCoder.normalize_method("huffman_adapt")
    rescale_slope = _first_or_value(getattr(header, "RescaleSlope", 1.0), 1.0)
    rescale_intercept = _first_or_value(getattr(header, "RescaleIntercept", 0.0), 0.0)
    window_center = _first_or_value(getattr(header, "WindowCenter", args.window_center), args.window_center)
    window_width = _first_or_value(getattr(header, "WindowWidth", args.window_width), args.window_width)
    ...
    if not output_path.lower().endswith(".mic"):
        output_path = f"{output_path}.mic"
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    if os.path.exists("output.mic"):
        os.replace("output.mic", output_path)
    else:
        raise FileNotFoundError("output.mic not found after encoding.")

    os.makedirs("result", exist_ok=True)
    result_mic = os.path.join("result", os.path.basename(output_path))
    if os.path.abspath(result_mic) != os.path.abspath(output_path):
        shutil.copyfile(output_path, result_mic)

    print(f"Saved compressed file: {output_path}")
    print(f"Copied compressed file: {result_mic}")


def main():
    parser = argparse.ArgumentParser(description="MIC encoder")
    parser.add_argument("--input", required=True, help="Path to DICOM file")
    parser.add_argument("--output", required=True, help="Output .mic path")
    parser.add_argument(
        "--quality",
        type=int,
        required=True,
    ...
    parser.add_argument("--window-center", type=float, default=40, help="Window center")
    parser.add_argument("--window-width", type=float, default=400, help="Window width")
